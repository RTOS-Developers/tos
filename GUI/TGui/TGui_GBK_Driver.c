...
{              
  uint32 Addr;
	...
	for(i=0;i<w;i++)
	{
		...
		Draw_Full_List((uint8 *)(Addr+i*P_TGUI_Font->SizeEH),P_TGUI_Font->SizeEH,X,Y,Color,P_TGui_Style->B_Color,P_TGui_Style->A_Color);
		X+=8;
	}
}
/****************************************************************************
...
{                                                 
    ...
    {    
        ...
        if(L>0x80){
            ...
            if(L=='\n'){X=SX;Y+=T;P_TGui_Style->SX=SX;P_TGui_Style->SY+=T;}
						else if(L=='\r');
            else {
        ...
        if(K>EX){X=SX;Y+=T;if(Y>EY)return;LineIndex++;if(LineIndex>Line)return;}
    }
}
