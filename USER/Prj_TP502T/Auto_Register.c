...
u8 Reg_SendInfor()
{
	uint8 Buf[300],Date[20];
	uint16 i,z=0;
R:
	i=0;
	i+=GetStringByFmt((char *)&Buf[i],"{");
	i+=GetStringByFmt((char *)&Buf[i],"\"method\":\"1\",");
	i+=GetStringByFmt((char *)&Buf[i],"\"ent_key\":\"35\",");
	i+=GetStringByFmt((char *)&Buf[i],"\"device_id\":\"\",");
	GetCodeStr16(Date,z),
	i+=GetStringByFmt((char *)&Buf[i],"\"device_no\":\"%s\",",Date);
	BufferCoppy(Date,&UIShowValue.SaveValue.LoginHead[0],sizeof(UIShowValue.SaveValue.LoginHead));
	...
	{
		DeBug("设备注册失败[%d]",z,Infor_Warning);
		z++;
	...
	if(USART_RX_Buffer[1][1]=='0')
	{
		...
		UIShowValue.SaveValue.DeBugCFG=0xff;
