...
/**************************************************************************************
 Func: TOS为需要配置参数的任务提供配置需求
 Time: 2014-11-13
 Ver.:	  	
 Note: 
**************************************************************************************/
void FlashConfigRoom_Init()
{
	FlashAddr_Index=0;
}
/**************************************************************************************
 ...
 Time: 2014-11-13
 Ver.:	  	
 Note: 申请配置空间的用户将通过Offset获取偏移位置，Size 为配置的长度，通常以双字节计数
**************************************************************************************/
void FlashGetConfigRoom(uint8 *Offset,uint8 Size)
{
	if(*Offset==FlashConfigNull)
	{
	*Offset=FlashAddr_Index;
	FlashAddr_Index+=Size;
	}
}
/**************************************************************************************
 Func: TOS挂在设备接口
 Time: 2014-11-13	
 Note: 挂在的设备注册到系统，系统会对设备进行统一初始化
**************************************************************************************/
void DeviceMount_Flash()
{
	DeviceId_FLASH=DeviceId_Index++;
	Tos_Device_Tab[DeviceId_FLASH].DeviceName="FLASH";
	Tos_Device_Tab[DeviceId_FLASH].DeviceOwnerId=Null;
	Tos_Device_Tab[DeviceId_FLASH].DeviceVirtue=DV_Task;
	Tos_Device_Tab[DeviceId_FLASH].DeviceState=NotBusy;
	Tos_Device_Tab[DeviceId_FLASH].Init=FlashConfigRoom_Init;
	Tos_Device_Tab[DeviceId_FLASH].Write=Flash_Write;
	Tos_Device_Tab[DeviceId_FLASH].Read=Flash_Read;
	Tos_Device_Tab[DeviceId_FLASH].Exit=Null;
}
